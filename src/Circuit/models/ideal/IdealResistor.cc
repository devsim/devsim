...
#include <cmath>
IdealResistor::IdealResistor(NodeKeeper *nk, const char *name, const char *n1,
                             const char *n2)
...
void IdealResistor::assembleDC_impl(const NodeKeeper::Solution &sol,
...
void IdealResistor::assembleTran_impl(
...
IdealResistor_create(NodeKeeper *nk, const std::string &name,
                     const std::vector<std::string> &nodelist) {
  return new IdealResistor(nk, name.c_str(), nodelist[0].c_str(),
                           nodelist[1].c_str());
}
