...
#include <cmath>
IdealCapacitor::IdealCapacitor(NodeKeeper *nk, const char *name, const char *n1,
                               const char *n2)
    : InstanceModel(nk, name) {
...
void IdealCapacitor::assembleDC_impl(const NodeKeeper::Solution &sol,
                                     dsMath::RealRowColValueVec<double> &mat,
                                     std::vector<std::pair<int, double>> &rhs) {

}

void IdealCapacitor::assembleTran_impl(
    const double scl, const NodeKeeper::Solution &sol,
    dsMath::RealRowColValueVec<double> *mat,
    std::vector<std::pair<int, double>> &rhs) {
  ...
  if (!is_gnd_node_vbot) {
    if (!is_gnd_node_vtop)
      mat->push_back(dsMath::RealRowColVal<double>(node_num_vbot, node_num_vtop,
                                                   evbot_vtop));
    mat->push_back(dsMath::RealRowColVal<double>(node_num_vbot, node_num_vbot,
                                                 evbot_vbot));
  }
  if (!is_gnd_node_vtop) {
    mat->push_back(dsMath::RealRowColVal<double>(node_num_vtop, node_num_vtop,
                                                 evtop_vtop));
    if (!is_gnd_node_vbot)
      mat->push_back(dsMath::RealRowColVal<double>(node_num_vtop, node_num_vbot,
                                                   evtop_vbot));
  }
}
...
IdealCapacitor_create(NodeKeeper *nk, const std::string &name,
                      const std::vector<std::string> &nodelist) {
  return new IdealCapacitor(nk, name.c_str(), nodelist[0].c_str(),
                            nodelist[1].c_str());
}
