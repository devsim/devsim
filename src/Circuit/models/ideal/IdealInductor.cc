...
#include <cmath>
IdealInductor::IdealInductor(NodeKeeper *nk, const char *name, const char *n1,
                             const char *n2)
...
void IdealInductor::assembleDC_impl(const NodeKeeper::Solution &sol,
  ...
  if (!is_gnd_node_I) {
    if (!is_gnd_node_vtop)
      mat.push_back(
          dsMath::RealRowColVal<double>(node_num_I, node_num_vtop, eI_vtop));
    if (!is_gnd_node_vbot)
      mat.push_back(
          dsMath::RealRowColVal<double>(node_num_I, node_num_vbot, eI_vbot));
  }
  if (!is_gnd_node_vbot) {
    if (!is_gnd_node_I)
      mat.push_back(
          dsMath::RealRowColVal<double>(node_num_vbot, node_num_I, evbot_I));
  }
  if (!is_gnd_node_vtop) {
    if (!is_gnd_node_I)
      mat.push_back(
          dsMath::RealRowColVal<double>(node_num_vtop, node_num_I, evtop_I));
  }
}

void IdealInductor::assembleTran_impl(
...
IdealInductor_create(NodeKeeper *nk, const std::string &name,
                     const std::vector<std::string> &nodelist) {
  return new IdealInductor(nk, name.c_str(), nodelist[0].c_str(),
                           nodelist[1].c_str());
}
