...
class MeshWriterTestFunctor {
 public:
  MeshWriterTestFunctor(ObjectHolder oh)
      : object_holder_(oh), call_function_(false)
  {
  }
  ...
  bool initialize(std::string &errorString)
  {
    ...
    if (object_holder_.IsCallable())
    {
      ...
      {
        errorString +=
            "\"include_test\" is not a callable one argument function\n";
...
bool MeshWriter::WriteMesh(const std::string &deviceName,
                           const std::string &filename, ObjectHolder test,
...
bool MeshWriter::WriteMeshes(const std::string &filename, ObjectHolder test,
                             std::string &errorString)
{
  MeshWriterTestFunctor test_functor(test);
  bool ret = test_functor.initialize(errorString);
