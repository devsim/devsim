...
template <typename DoubleType> using ComplexDouble_t = std::complex<DoubleType>;
...
template <typename T, class U> void CopySTLVectorToRaw(T *nv, const U &ov) {
  T *p = nv;
...
template <typename T, class U> void CopyRawToSTLVector(T *nv, U &ov) {
  T *p = nv;
