...
Newton<DoubleType>::Newton()
    : maxiter(DefaultMaxIter), absLimit(DefaultAbsError),
      relLimit(DefaultRelError), maxLimit(DefaultMaxAbsError),
      qrelLimit(DefaultQRelError), dimension(0) {}
...
template <typename DoubleType>
void Newton<DoubleType>::LoadCircuitRHSAC(
    std::vector<std::complex<DoubleType>> &rhs) {

  typedef std::vector<std::pair<size_t, std::complex<double>>>
  ...
  if (nk.HaveNodes()) {
    ...
    for (typename ComplexRHSEntryVec_t::iterator it = cv.begin();
         it != cv.end(); ++it) {
      rhs[it->first + offset] =
          std::complex<DoubleType>(static_cast<DoubleType>(it->second.real()),
...
template <typename DoubleType>
void Newton<DoubleType>::LoadMatrixAndRHSAC(
    Matrix<DoubleType> &matrix, std::vector<std::complex<DoubleType>> &rhs,
    permvec_t &permvec, DoubleType frequency) {
#if 1
...
#endif
#endif
  const std::complex<DoubleType> jOmega =
      two_pi * std::complex<DoubleType>(0, 1.0) * frequency;
  ...
  if (dbent.first) {
    ...
    if (val == "superlu") {
...
#else
      std::ostringstream os;
      os << "Solver \"mkl_pardiso\" not supported in this build.  Switching to "
            "\"superlu\".\n";
      OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
#endif
    } else {
      ...
      os << "Unrecognized \"direct_solver\" parameter value \"" << val
         << "\". Valid options are \"mkl_pardiso\" or \"superlu\".\n";
      OutputStream::WriteOut(OutputStream::OutputType::INFO, os.str());
    }
  }
...
CreatePreconditioner(LinearSolver<float128> &itermethod, size_t numeqns) {
  ...
  if (dynamic_cast<IterativeLinearSolver<float128> *>(&itermethod)) {
    preconditioner = new BlockPreconditioner<float128>(
        numeqns, PEnum::TransposeType_t::NOTRANS);
  } else {
    if (auto s = GetDirectSolver(); s == DirectSolver::SUPERLU) {
      preconditioner = new SuperLUPreconditioner<float128>(
          numeqns, PEnum::TransposeType_t::NOTRANS, PEnum::LUType_t::FULL);
    }
#ifdef USE_MKL_PARDISO
    else if (s == DirectSolver::MKLPARDISO) {
      preconditioner = new MKLPardisoPreconditioner<float128>(
          numeqns, PEnum::TransposeType_t::NOTRANS);
    }
#endif
    else {
...
CreateMatrix(Preconditioner<DoubleType> *preconditioner,
...
#ifdef USE_MKL_PARDISO
  else if (dynamic_cast<MKLPardisoPreconditioner<DoubleType> *>(
               preconditioner)) {
    compression_type = CompressionType::CCM;
  }
#endif
...
void CallACUpdateSolution(NodeKeeper &nk, const std::string &rname,
                          const std::string &iname,
                          std::vector<std::complex<float128>> &result) {
  std::vector<std::complex<double>> tmp(result.size());
  for (size_t i = 0; i < result.size(); ++i) {
    tmp[i] = std::complex<double>(static_cast<double>(result[i].real()),
...
void Newton<DoubleType>::GetMatrixAndRHSForExternalUse(CompressionType ct,
  ...
  for (const auto &p : loads) {
    ...
    if (!matrix) {
      matrix = std::unique_ptr<CompressedMatrix<DoubleType>>(
          new CompressedMatrix<DoubleType>(numeqns, matrix_type, ct));
    }
...
void Newton<DoubleType>::InitializeTransientAssemble(
    ...
    std::vector<DoubleType> &rhs_constant) {
...
bool Newton<DoubleType>::CheckTransientProjection(
    const TimeMethods::TimeParams<DoubleType> &timeinfo,
    const std::vector<DoubleType> &newQ, ObjectHolderMap_t *ohm) {
...
void Newton<DoubleType>::UpdateTransientCurrent(
    const TimeMethods::TimeParams<DoubleType> &timeinfo, size_t numeqns,
    const std::vector<DoubleType> &newI, const std::vector<DoubleType> &newQ) {
  TimeData<DoubleType> &tinst = TimeData<DoubleType>::GetInstance();
  if (timeinfo.IsDCMethod()) {
