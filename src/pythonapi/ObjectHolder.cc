  ...
  if (PyBytes_Check(obj)) {
    ...
    if (bytecall.IsCallable()) {
      bytes = ObjectHolder(PyObject_CallObject(
          reinterpret_cast<PyObject *>(bytecall.GetObject()), NULL));
      PyErr_Clear();
    }
  }
}
...
ObjectHolder::ObjectHolder(double v) { object_ = PyFloat_FromDouble(v); }

ObjectHolder::ObjectHolder(int v) { object_ = PyLong_FromLong(v); }
...
template <typename T> ObjectHolder CreatePODArray(const std::vector<T> &list) {
  ...
  return CreateArrayObject(pod_info<T>::ptype,
                           ObjectHolder(list.data(), length));
}
