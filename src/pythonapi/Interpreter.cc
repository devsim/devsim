...
ObjectHolder CreateDictionary(
    const std::vector<std::pair<std::string, ObjectHolder>> &objects) {
  ObjectHolder ret;
  ...
  for (size_t i = 0; i < objects.size(); ++i) {
    ...
    const PyObject *obj =
        reinterpret_cast<const PyObject *>((objects[i].second).GetObject());
...
bool Interpreter::RunCommand(
  ...
  PyObject *robj =
      PyObject_Call(reinterpret_cast<PyObject *>(command_object.GetObject()),
                    args, reinterpret_cast<PyObject *>(kwargs.GetObject()));
  result_ = ObjectHolder(robj);
  if (robj) {
    ret = true;
  } else {
